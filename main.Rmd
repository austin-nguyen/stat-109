...
trans_data$t_accommodates <- trans_data$accommodates^(-0.04844065)
trans_data$t_host_since_tenure <- trans_data$host_since_tenure^(0.72619421)
trans_data$t_guests_included <- trans_data$guests_included^(-1.99328162)
...
trans_data <- subset(trans_data, select = -c(accommodates,host_since_tenure, guests_included))
...
#p-value is 0.07961, telling us we cannot reject the null hypothesis that the additional coefficients are equal to zero. This means that we stick with the reduced model.  
...
#p-value is  0.03666, meaning that the fuller model with interaction terms is better 
full_model_interact <- lm(log_price ~ ., trans_interact_data)
anova(full_model,full_model_interact)

#p-value is 0.003404, meaning that the fuller model with interaction terms is better 
